...
**⚠️ 重要**: 小说和剧本是不同的文体,需要格式转换!

| 小说元素 | 转换方式 | 剧本格式示例 |
|---------|---------|------------|
| **心理描写** | → 动作外化 或 os | `徐溺 os:他在说谎` |
| **环境描写** | → 场景标注 + 简短△ | `场1-1 咖啡厅 日 内`<br>`△温暖的午后阳光透过窗户洒进来` |
| **大段叙述** | → 拆分成多场景 | 每个场景独立标注 |
| **内心独白** | → os + 特写 | `徐溺 os:为什么要这样对我` |
| **回忆插叙** | → 【闪回】【闪出】 | `【闪回】`<br>`△三年前的场景`<br>`【闪出】` |
| **旁白叙述** | → VO 或 删除 | `VO(旁白):就这样,故事开始了` |
